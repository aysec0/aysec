...
export function announceProLabFirstBlood({ userDisplay, userUsername, labTitle, labSlug, machineName, flagKind, points }) {
  return send({
    title: flagKind === 'root' ? '👑 Pro Lab — first ROOT' : '🩸 Pro Lab — first foothold',
    description: `**${userDisplay || userUsername}** is the first to capture the **${flagKind}** flag on **${machineName}** in **${labTitle}**.`,
    url: `${SITE_URL}/pro-labs/${labSlug}`,
    color: flagKind === 'root' ? COLOR.blood : COLOR.warning,
    fields: [
      { name: 'machine',  value: machineName, inline: true },
      { name: 'flag',     value: flagKind,    inline: true },
      { name: 'points',   value: `+${points}`, inline: true },
      { name: 'player',   value: `[${userDisplay || userUsername}](${SITE_URL}/u/${userUsername})`, inline: true },
    ],
    footer: { text: 'aysec pro labs — pivot harder' },
    timestamp: new Date().toISOString(),
  });
}
